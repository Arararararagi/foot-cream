...
local function _conv_prime_to_m(text)
    ...
    if not f then f, i = clean:match("^([0-9]+)" .. FootFree._CAPOS .. "([0-9]+)") end
    if f and i then
...
local _PRIME_PATS = {
    { pat = _ND.."(".._PRIME.."|'|"..FootFree._CAPOS..")[ ]*".._TIMES.."[ ]*[0-9]+(".._DPRIME.."|\"|"..FootFree._CDQ..")",
      converter = _conv_dim_to_m_cm, target = "m×cm", cat = "length" },
    { pat = _ND.."(".._PRIME.."|'|"..FootFree._CAPOS..")[0-9]+(".._DPRIME.."|\"|"..FootFree._CDQ..")",
      converter = _conv_prime_to_m, target = "m", cat = "length" },
    { pat = _ND.."(".._PRIME.."|'|"..FootFree._CAPOS..")",   factor = 0.3048, offset = 0, target = "m",  cat = "length" },
    { pat = _ND.."(".._DPRIME.."|\"|"..FootFree._CDQ..")", factor = 2.54,   offset = 0, target = "cm", cat = "length" },
}
...
function FootFree:_finishScan(doc, all_matches, t_per_pat, t_total, in_subprocess, debug_report)
    ...
    for _, r in ipairs(all_matches) do
        ...
        local _ends_single_apos = _mt_end1 == "'" or r.matched_text:sub(-3) == _PRIME
            or r.matched_text:sub(-3) == FootFree._CAPOS
